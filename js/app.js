(() => {
  ...
  const state = {
    ...
    matchResults: load("arnomendi_match_results_v1", DATA.matchResults || {}),
    onlineReady: false,
    onlineError: ""
  };
  ...
  async function loadOnlineState() {
    try {
      ...
      if (row) {
        ...
        state.matchResults = row.match_results && typeof row.match_results === "object" ? row.match_results : {};
      } else {
        state.users = [];
    ...
    localStorage.setItem("arnomendi_match_results_v1", JSON.stringify(state.matchResults));
      localStorage.setItem("arnomendi_match_results_v1", JSON.stringify(state.matchResults));
  ...
  async function syncOnlineState() {
    const payload = {
      ...
      events: state.events,
      bets: state.bets,
      match_results: state.matchResults,
  ...
  const SPAIN_SCORERS = [
    "Ferran Torres", "Mikel Oyarzabal", "Lamine Yamal", "Nico Williams", "Borja Iglesias", "Víctor Muñoz", "Yeremi Pino", "Dani Olmo",
    "Pedri", "Rodri", "Fabián Ruiz", "Martín Zubimendi", "Gavi", "Mikel Merino", "Álex Baena",
    "Pau Cubarsí", "Pedro Porro", "Marc Cucurella", "Aymeric Laporte", "Alejandro Grimaldo", "Marcos Llorente", "Marc Pubill", "Eric García",
    "Unai Simón", "Joan García", "David Raya", "No marca ningún español"
  ];
  ...
  const matchResult = (matchNo) => state.matchResults?.[String(matchNo)] || null;

  function resultText(matchNo) {
    const r = matchResult(matchNo);
    return r ? `${Number(r.homeGoals)}-${Number(r.awayGoals)}` : "";
  }

  function setMatchResult(matchNo, homeGoals, awayGoals) {
    const m = DATA.fixtures.find(x => String(x.no) === String(matchNo));
    if (!m || !/^[A-L]$/.test(String(m.group))) return false;
    const hg = Number(homeGoals), ag = Number(awayGoals);
    if (!Number.isInteger(hg) || !Number.isInteger(ag) || hg < 0 || ag < 0) return false;
    state.matchResults[String(matchNo)] = {
      matchNo: Number(m.no),
      home: m.home,
      away: m.away,
      homeGoals: hg,
  ...
  function clearMatchResult(matchNo) {
    if (state.matchResults) delete state.matchResults[String(matchNo)];
  }

  function groupStandings(groupId) {
    const group = DATA.groups.find(g => g.group === groupId);
    if (!group) return [];
    const table = Object.fromEntries(group.teams.map(name => [name, { team: name, played: 0, won: 0, drawn: 0, lost: 0, gf: 0, ga: 0, gd: 0, pts: 0 }]));
    DATA.fixtures
      .filter(m => m.group === groupId)
      .forEach(m => {
        const r = matchResult(m.no);
        if (!r || !table[m.home] || !table[m.away]) return;
  ...
  function worldcup() {
    return `<div class="grid"><section class="card"><span class="badge">🏆 Datos del Mundial</span><div class="layout" style="align-items:end;margin-top:16px"><div><h1 class="section-title" style="font-size:44px">Mundial 2026</h1><p class="section-sub">Grupos, participantes, resultados y calendario completo en horario peninsular español.</p></div><div class="grid grid3">${stat("🌍", "Selecciones", 48, "Participantes")}${stat("📊", "Grupos", 12, "A-L")}${stat("✅", "Resultados", Object.keys(state.matchResults || {}).length, "Introducidos")}</div></div></section><section class="layout"><div class="card"><h2 class="section-title">Clasificación de grupos</h2><p class="section-sub">Se actualiza automáticamente cuando el administrador introduce resultados.</p><div class="group-grid standings-grid">${DATA.groups.map(groupCard).join("")}</div></div><div class="card"><span class="badge">🇪🇸 Camino de España</span><h2 class="section-title" style="margin-top:14px">Partidos de España</h2><p class="section-sub">Grupo H · horario de España.</p><div class="fixtures" style="margin-top:16px">${DATA.fixtures.filter(isSpain).map(m => fixture(m, true)).join("")}</div></div></section><section class="card"><div class="layout" style="align-items:end"><div><h2 class="section-title">Calendario completo</h2><p class="section-sub">Fase de grupos y eliminatorias.</p></div><label class="search">🔎<input id="fixtureSearch" placeholder="Buscar equipo, grupo o sede..."></label></div><div class="filters">${["Todos", "España", ...DATA.groups.map(g => g.group), "Eliminatorias"].map((f, i) => `<button class="filter ${i === 0 ? "active" : ""}" data-filter="${e(f)}">${f.length === 1 ? `Grupo ${f}` : f}</button>`).join("")}</div><div class="scroll fixtures">${DATA.fixtures.map(m => fixture(m)).join("")}</div></section></div>`;
  }

  function groupCard(g) {
    const rows = groupStandings(g.group);
    return `<div class="group-card ${g.group === "H" ? "spain" : ""}"><div class="group-head"><strong>Grupo ${g.group}</strong>${g.group === "H" ? `<span class="badge">España</span>` : ""}</div><table class="group-table"><thead><tr><th>Equipo</th><th>PJ</th><th>DG</th><th>Pts</th></tr></thead><tbody>${rows.map((t, i) => `<tr><td><span class="stand-pos">${i + 1}</span> ${flag(t.team)} ${e(t.team)}</td><td>${t.played}</td><td>${t.gd > 0 ? "+" : ""}${t.gd}</td><td><strong>${t.pts}</strong></td></tr>`).join("")}</tbody></table></div>`;
  }

  function fixture(m, compact = false) {
    const search = `${m.home} ${m.away} ${m.group} ${m.venue}`.toLowerCase();
    const r = resultText(m.no);
    return `<div class="fixture ${isSpain(m) ? "spain" : ""} ${r ? "played" : ""}" data-fixture data-search="${e(search)}" data-group="${e(m.group)}"><div class="fixture-grid"><div><div class="muted">Partido</div><div class="fixture-no">${m.no}</div></div><div><strong>${e(m.date)}</strong><div class="fixture-time">${e(m.time)} h</div></div><div><div>${team(m.home)} <strong class="match-score ${r ? "has-result" : ""}">${r || "vs"}</strong> ${team(m.away)}</div><div class="venue">📍 ${e(m.venue)}</div></div>${compact ? "" : `<div class="fixture-stage"><span class="badge">${e(stage(m.group))}</span>${r ? `<div class="result-chip">Finalizado</div>` : ""}</div>`}</div></div>`;
  }
  ...
  function bind() {
    ...
    const realMatch = $("#realMatch"), realHomeGoals = $("#realHomeGoals"), realAwayGoals = $("#realAwayGoals");
    if (realMatch) {
      const preload = () => {
        const r = matchResult(realMatch.value);
        realHomeGoals.value = r ? Number(r.homeGoals) : "";
        realAwayGoals.value = r ? Number(r.awayGoals) : "";
      };
      realMatch.onchange = preload;
      preload();
    }
    const saveMatchResult = $("#saveMatchResult");
    if (saveMatchResult) saveMatchResult.onclick = () => {
      if (realHomeGoals.value === "" || realAwayGoals.value === "" || !setMatchResult(realMatch.value, Number(realHomeGoals.value), Number(realAwayGoals.value))) return alert("Introduce un resultado válido para un partido de la fase de grupos.");
      save();
      render();
    };
    const clearMatchResultBtn = $("#clearMatchResult");
    if (clearMatchResultBtn) clearMatchResultBtn.onclick = () => {
      const m = DATA.fixtures.find(x => String(x.no) === String(realMatch.value));
      if (!m || !confirm(`¿Borrar el resultado de ${m.home} vs ${m.away}?`)) return;
      clearMatchResult(realMatch.value);
      save();
      render();
    };
